...
const Dashboard: React.FC<DashboardProps> = ({ user }) => {
  ...
  const refreshData = async (showLoading = true) => {
    ...
    try {
      ...
      const sortedItems = cloudItems.sort((a, b) => b.timestamp - a.timestamp);
      ...
      console.error("Master DB Sync Error", e);
    } finally {
  ...
  const markAsHandovered = async (itemToUpdate?: FoundItem) => {
    ...
    if (item.founderId.toUpperCase() !== user.id.toUpperCase()) {
      alert("Founder access only.");
      return;
    }

    if (confirm("Confirm item returned? ✅")) {
      setIsRefreshing(true);
      try {
        const updatedItem: FoundItem = { ...item, status: 'handovered' };
        setItems(prev => prev.map(i => i.id === item.id ? updatedItem : i));
        if (selectedItem?.id === item.id) setSelectedItem(updatedItem);
        if (newlyCreatedItem?.id === item.id) setNewlyCreatedItem(updatedItem);
        
        await db.updateItem(updatedItem);
        triggerToast("Item returned! ✅");
        
        if (uploadStep === 'thankYou') {
  ...
  const handleDeletePost = async (itemId: string) => {
    if (!confirm("Are you sure you want to delete this post permanently? ⚠️")) return;
    ...
    try {
      ...
      triggerToast("Post Deleted Successfully 🗑️");
    } catch (err) {
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      {showToast.show && (
        <div className="fixed top-24 left-1/2 -translate-x-1/2 z-[300] animate-in slide-in-from-top-4">
          <div className="bg-indigo-900 text-white px-8 py-4 rounded-[2rem] shadow-2xl flex items-center space-x-3 border-2 border-white/20">
      ...
      {isRefreshing && (
        <div className="fixed bottom-8 right-8 bg-white/90 backdrop-blur px-4 py-2 rounded-full shadow-lg border border-gray-100 z-50 flex items-center space-x-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="bg-white p-8 rounded-[3.5rem] shadow-2xl border-t-[8px] border-indigo-600 sticky top-24">
            ...
            {uploadStep === 'preview' && capturedImage && (
              <div className="space-y-6 animate-in fade-in">
                ...
                <button onClick={() => setUploadStep('none')} className="w-full text-gray-400 py-2 font-black text-[10px] uppercase tracking-widest">Discard</button>
              </div>
            )}
            ...
            {uploadStep === 'vivaChoice' && (
              <div className="space-y-4 text-center py-4">
                ...
                <div className="flex flex-col gap-3">
                  <button onClick={() => setUploadStep('manualViva')} className="bg-white border-2 border-indigo-600 text-indigo-600 py-5 rounded-[2rem] font-black text-sm hover:bg-indigo-50 transition-all">Manual Security</button>
                  <button onClick={() => finalizeUpload(true)} className="bg-indigo-600 text-white py-5 rounded-[2rem] font-black text-sm shadow-lg hover:bg-indigo-700">AI Intelligent Scan</button>
                </div>
              </div>
            )}
              ...
              <div className="space-y-4 animate-in fade-in">
                {manualQuestions.map((mq, idx) => (
                  <div key={idx} className="space-y-2 p-4 bg-gray-50 rounded-[2rem] border border-gray-100">
                    <input type="text" placeholder="Question" className="w-full bg-white px-4 py-3 rounded-xl border-none outline-none font-bold text-xs"
                      value={mq.q} onChange={e => { const n = [...manualQuestions]; n[idx].q = e.target.value; setManualQuestions(n); }} />
                    <input type="text" placeholder="Answer" className="w-full bg-white px-4 py-3 rounded-xl border-none outline-none font-bold text-xs"
                      value={mq.a} onChange={e => { const n = [...manualQuestions]; n[idx].a = e.target.value; setManualQuestions(n); }} />
                  </div>
                ))}
                <button onClick={() => setManualQuestions([...manualQuestions, { q: '', a: '' }])} className="text-[10px] font-black text-indigo-500 py-1 w-full text-center">+ Add Question</button>
                <button onClick={() => finalizeUpload(false)} disabled={isUploading || manualQuestions.some(m => !m.q || !m.a)} className="w-full bg-indigo-600 text-white py-6 rounded-[2rem] font-black shadow-lg">
                  {isUploading ? 'Securing...' : 'Submit Report'}
                </button>
              </div>
            )}
            ...
            {uploadStep === 'thankYou' && newlyCreatedItem && (
              <div className="text-center py-6 animate-in zoom-in">
                ...
                <h4 className="text-3xl font-black text-gray-900 mb-2">Honesty Recorded</h4>
                <p className="text-gray-400 font-bold text-[10px] uppercase tracking-widest mb-8 leading-relaxed">Synced across the network</p>
                
                <div className="space-y-3">
                  {newlyCreatedItem.status === 'available' ? (
                    <button 
                      onClick={() => markAsHandovered(newlyCreatedItem)} 
                      className="w-full py-6 bg-green-600 text-white rounded-[1.5rem] font-black text-xs uppercase tracking-widest shadow-xl hover:bg-green-700 active:scale-95 flex items-center justify-center space-x-2 border-2 border-white/20"
                    >
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="4" d="M5 13l4 4L19 7"/></svg>
                      <span>Mark as Returned ✅</span>
                    </button>
                  ) : (
                    <div className="w-full py-6 bg-green-50 text-green-700 rounded-[1.5rem] font-black text-xs uppercase tracking-widest border-2 border-green-200">
                      Case Resolved ✅
                    </div>
                  )}
                  <button onClick={() => handleDeletePost(newlyCreatedItem.id)} className="w-full py-4 bg-red-50 text-red-600 rounded-[1.5rem] font-black text-[10px] uppercase tracking-widest border border-red-100 hover:bg-red-100 transition-all">Delete Post 🗑️</button>
                  <button onClick={() => { setUploadStep('none'); setCapturedImage(null); setNewlyCreatedItem(null); }} className="w-full py-4 bg-gray-900 text-white rounded-[1.5rem] font-black text-[10px] uppercase tracking-widest">Home Feed</button>
                </div>
              </div>
            )}
        ...
        <div className="lg:col-span-8 space-y-8">
          <div className="flex items-center justify-between px-4">
            <h3 className="text-3xl font-black text-gray-900 tracking-tighter">Discovery Feed</h3>
            <button onClick={() => refreshData()} className="p-3 text-indigo-600 hover:bg-indigo-50 rounded-2xl transition-all">
              <svg className={`w-6 h-6 ${isRefreshing ? 'animate-spin' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>
            </button>
          </div>
                ...
                <div className="relative aspect-square overflow-hidden bg-black">
                  ...
                  {item.status === 'available' ? (
                    <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm p-8">
                      <button onClick={() => { setSelectedItem(item); setUserAnswers(new Array(item.verificationQuestions.length).fill('')); setVerificationResult('idle'); }} className="w-full bg-white text-gray-900 py-5 rounded-[1.5rem] font-black text-xs uppercase tracking-widest shadow-2xl">Verify & Claim</button>
                    </div>
                  ) : (
                ...
                <div className="p-8 flex-grow flex flex-col justify-between space-y-4">
                  <div>
                    <div className="flex items-center space-x-3 mb-2">
                      <div className={`w-3 h-3 rounded-full ${item.status === 'available' ? 'bg-green-500 animate-pulse' : 'bg-gray-300'}`}></div>
                      <p className="text-xl font-black text-gray-900 tracking-tight leading-none">Found by {item.founderName}</p>
                    </div>
                  ...
                  {item.founderId.toUpperCase() === user.id.toUpperCase() && (
                    <div className="pt-2 space-y-2">
                      {item.status === 'available' && (
                        <button 
                          onClick={(e) => { e.stopPropagation(); markAsHandovered(item); }} 
                          className="w-full bg-green-600 text-white py-4 rounded-[1.5rem] font-black text-[11px] shadow-xl hover:bg-green-700 active:scale-95 tracking-widest uppercase flex items-center justify-center space-x-2 border-2 border-white/10"
                        >
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="4" d="M5 13l4 4L19 7"/></svg>
                          <span>Confirm Item Returned ✅</span>
                        </button>
                      )}
                      <button 
                        onClick={(e) => { e.stopPropagation(); handleDeletePost(item.id); }} 
                        className="w-full bg-red-50 text-red-600 py-3 rounded-[1.25rem] font-black text-[9px] tracking-widest uppercase flex items-center justify-center space-x-2 border border-red-100 hover:bg-red-100 active:scale-95"
                      >
                        ...
                        <span>Delete Report 🗑️</span>
                      </button>
                      <p className="text-[8px] text-center text-gray-400 font-black uppercase tracking-[0.2em] mt-1 opacity-60">Founder Privileges</p>
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="bg-white w-full max-w-xl rounded-[4rem] p-10 max-h-[90vh] overflow-y-auto relative shadow-2xl border border-gray-100">
            ...
            {verificationResult === 'idle' && selectedItem.founderId.toUpperCase() !== user.id.toUpperCase() && selectedItem.status === 'available' && (
              <div className="space-y-8 py-4">
                <div className="text-center">
                  ...
                  <h4 className="text-3xl font-black text-gray-900 mb-2">Ownership Portal</h4>
                  <p className="text-gray-400 font-bold text-xs uppercase tracking-widest">Verify credentials to unlock chat</p>
                </div>
                {selectedItem.verificationQuestions.map((q, idx) => (
                  <div key={idx} className="space-y-3">
                    <label className="block text-[10px] font-black text-indigo-600 uppercase tracking-widest px-2">Challenge {idx + 1}</label>
                    <p className="text-xl font-bold text-gray-800 px-2">{q}</p>
                ...
                <button onClick={handleClaim} disabled={isVerifying || userAnswers.some(a => !a.trim())} className="w-full bg-indigo-600 text-white font-black py-6 rounded-[2rem] shadow-xl text-xl tracking-tighter">
                  {isVerifying ? 'Verifying...' : 'Validate Access'}
                </button>
              </div>
            )}
            ...
            {(verificationResult === 'success' || selectedItem.founderId.toUpperCase() === user.id.toUpperCase() || selectedItem.status === 'handovered') && (
              <div className="flex flex-col h-[75vh] animate-in slide-in-from-bottom-8">
                <div className="flex-shrink-0 text-center pb-8 border-b border-gray-100">
                  <h5 className="text-3xl font-black text-gray-900 tracking-tighter mb-6">Management Tunnel</h5>
                  <div className="flex flex-col items-center gap-4">
                    <div className="flex items-center space-x-3 bg-indigo-50 px-6 py-3 rounded-full border border-indigo-100">
                       <svg className="w-5 h-5 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" strokeWidth="3"/></svg>
                       <span className="text-sm font-black text-indigo-700 tracking-widest">{selectedItem.founderPhone}</span>
                    </div>
                    
                    {selectedItem.founderId.toUpperCase() === user.id.toUpperCase() && (
                      <div className="w-full p-6 bg-gray-50 rounded-[2.5rem] border-2 border-dashed border-gray-200 shadow-inner flex flex-col gap-3">
                        {selectedItem.status === 'available' && (
                          <button 
                            onClick={() => markAsHandovered()} 
                            className="w-full bg-green-600 text-white px-8 py-5 rounded-[1.5rem] text-[15px] font-black uppercase tracking-widest shadow-xl hover:bg-green-700 active:scale-95 flex items-center justify-center space-x-3 border-2 border-white/10"
                          >
                            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="5" d="M5 13l4 4L19 7"/></svg>
                            <span>Confirm Item Returned ✅</span>
                          </button>
                        ...
                        <button 
                          ...
                          className="w-full bg-red-50 text-red-600 py-4 rounded-[1.5rem] text-[12px] font-black uppercase tracking-widest border border-red-100 hover:bg-red-100 active:scale-95"
                        >
                          Delete This Post 🗑️
                        </button>
                      </div>
                    ...
                    {selectedItem.status === 'handovered' && (
                      <div className="py-4 px-12 bg-green-600 text-white rounded-[2rem] font-black text-lg uppercase tracking-widest shadow-lg flex items-center space-x-3">
                        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="5" d="M5 13l4 4L19 7"/></svg>
                ...
                <div className="flex-grow overflow-y-auto py-8 space-y-8 px-6 bg-gray-50/50 rounded-[3rem] my-8 border border-gray-100 shadow-inner">
                  {selectedItem.messages.length === 0 ? (
                    <div className="text-center py-16 px-6">
                      <p className="text-indigo-500 font-black text-[11px] uppercase tracking-widest mb-4">Secure Channel Active</p>
                      <p className="text-sm text-gray-400 font-bold leading-relaxed opacity-70">Identity confirmed. Coordinate the handover safely through this encrypted tunnel.</p>
                    </div>
                  ) : (
                    selectedItem.messages.map((m, i) => (
                      <div key={i} className={`flex flex-col ${m.senderId === user.id ? 'items-end' : 'items-start'}`}>
                        <div className={`max-w-[85%] px-8 py-5 rounded-[2rem] shadow-lg ${m.senderId === user.id ? 'bg-indigo-600 text-white rounded-br-none' : 'bg-white text-gray-900 rounded-bl-none border border-gray-100'}`}>
                          <p className="text-lg font-bold leading-tight tracking-tight">{m.text}</p>
                        </div>
                        <span className="text-[10px] text-gray-400 mt-3 font-black uppercase tracking-widest px-4">{m.senderName} • {new Date(m.timestamp).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                      </div>
                    ))
                  )}
                  <div ref={chatEndRef} />
                ...
                {selectedItem.status === 'available' ? (
                  <div className="flex-shrink-0 pt-2 flex space-x-4 bg-white">
                    <input type="text" className="flex-grow px-8 py-5 bg-gray-50 border-2 border-gray-100 rounded-[2rem] outline-none font-black text-lg focus:border-indigo-500/20 shadow-inner" placeholder="Secure message..."
                      value={newMessage} onChange={(e) => setNewMessage(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && sendMessage()} />
                    <button onClick={sendMessage} className="bg-indigo-600 text-white w-20 h-20 rounded-[2rem] shadow-xl flex items-center justify-center transition-all active:scale-90">
                      <svg className="w-8 h-8 rotate-45 -translate-x-0.5" fill="currentColor" viewBox="0 0 20 20"><path d="M10.894 2.553a1 1 0 00-1.788 0l-7 14a1 1 0 001.169 1.409l5-1.429A1 1 0 009 15.571V11a1 1 0 112 0v4.571a1 1 0 00.725.962l5 1.428a1 1 0 001.17-1.408l-7-14z"/></svg>
                    </button>
                  </div>
                ) : (
                  <div className="flex-shrink-0 pt-2 text-center opacity-50">
                    <p className="text-[10px] font-black uppercase tracking-widest">Handover Complete - Comms Locked ✅</p>
                  </div>
                )}
              </div>
            )}
            ...
            {verificationResult === 'fail' && (
              <div className="text-center py-20 animate-in zoom-in-95">
                <div className="w-32 h-32 bg-red-50 text-red-500 rounded-[3rem] flex items-center justify-center mx-auto mb-10 shadow-inner border-2 border-red-100">
                  <svg className="w-16 h-16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M6 18L18 6M6 6l12 12" strokeWidth="6"/></svg>
                </div>
                <h5 className="text-5xl font-black text-gray-900 mb-6 tracking-tighter">Access Denied</h5>
                <p className="text-gray-400 font-bold px-12 leading-relaxed text-xl mb-12 uppercase tracking-widest">Integrity check failed. Access restricted.</p>
                <button onClick={() => { setVerificationResult('idle'); setUserAnswers(new Array(selectedItem.verificationQuestions.length).fill('')); }} className="w-full py-8 bg-gray-900 text-white rounded-[2.5rem] font-black text-2xl shadow-xl transition-all active:scale-95">Retry Verification</button>
              </div>
            )}
          </div>
        </div>
